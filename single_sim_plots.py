...
sim_name = "single6_wide_equalmass_ecc0"
...
a_mavg = np.zeros((10000, n_sims))
e_mavg = np.zeros((10000, n_sims))
i_mavg = np.zeros((10000, n_sims))
...
for i in range(n_sims):
    ...
    inc = np.rad2deg(data[::skip,2])
    inc[inc <= 0.001] = np.nan
    inc[inc >= 100] = np.nan
    if (np.isnan(inc).any()):
        i_ended[i] = [np.where(np.isnan(inc))[0][0]-1, inc[np.where(np.isnan(inc))[0][0]-1]]
    
    a_mavg[:,i] = [np.mean(a[ii-10:ii]) for ii in range(len(a))]
    e_mavg[:,i] = [np.mean(e[ii-10:ii]) for ii in range(len(e))]
    i_mavg[:,i] = [np.mean(inc[ii-10:ii]) for ii in range(len(inc))]
...
for i in range(n_sims):
    ...
    axes[0].plot(x_ticks[::1000], a_mavg, lw=1.5, alpha=1)
    ...
    axes[1].plot(x_ticks[::1000], e_mavg, lw=1.5, alpha=1)
    ...
    axes[2].plot(x_ticks[::1000], (1-e_mavg)*a_mavg, lw=1.5, alpha=1)
    ...
    axes[3].plot(x_ticks[::1000], i_mavg, lw=1.5, alpha=1)
